...
$(document).ready(function()
{
  ...
  // checking if user uploaded via s3
  checkImageUpload();

  // dragger
  $dragger.draggable({
    drag: function(event) {
      var draggerLeftInverted = parseInt($dragger.css('left')) * (-1) + 'px';
      var draggerSetter = window.USER_IMAGE_ROTATED ? $dragger.css('top')+ ' ' + draggerLeftInverted : $dragger.css('left')+' '+$dragger.css('top');
      $userimage.css('background-position',draggerSetter);
  ...
  // image rotator; only one direction 90 degree
  // because on mobile it uploads 
  $rotator.click(function() {
    $userimage.addClass("transform");
    window.USER_IMAGE_ROTATED = true;
    $rotator.hide();
  })
...
// checking if image has already been uploaded to s3
function checkImageUpload() {
  setInterval(function() {
    if (window.S3_USER_IMAGE_UPLOAD && !window.S3_USER_HAS_UPLOADED) {
      var img = window.S3_USER_IMAGE_UPLOAD;;
      imageLoaded(img)();
      window.S3_USER_HAS_UPLOADED = true;
    }
  }, 1000)
}
